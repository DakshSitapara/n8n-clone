...
export const telegramExecutor: NodeExecutor<TelegramData> = async ({
    ...
    const chatIdRaw = data.chatId ? Handlebars.compile(data.chatId)(context) : undefined
    ...
    try {
        const result = await step.run('telegram-send', async () => {
            ...
            await ky.post(`https://api.telegram.org/bot${token}/${data.method}`, { json: payload })
...
function throwError(nodeId: string, publish: any, message: string): never {
